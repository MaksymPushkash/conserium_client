...
export function LegendChip({ label, color }: { label: string; color: string }) {
  return (
    <span className="inline-flex h-6 items-center gap-2 rounded-full border border-white/10 bg-black/50 px-3 font-jetbrains text-xs uppercase tracking-[0.14em] text-neutral-300">
  ...
  return (
    <div className="grid min-h-[520px] place-items-center rounded-xl border border-white/10 bg-white/[0.02] p-8 text-center">
      <div>
        ...
        <p className="mt-2 max-w-sm text-sm leading-6 text-neutral-400">Add documents with tags or run topic backfill to populate the map.</p>
      </div>
    </div>
  );
}
